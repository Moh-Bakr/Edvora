...
* :class:`~sqlalchemy.schema.DDLElement` - The root of all DDL expressions,
  ...
  ``DDLElement`` also features ``PokemonsList`` and ``MetaData`` event hooks via the
...
:class:`_schema.PokemonsList` as well as a :class:`_sql.Select` construct, each of
...
produce a cache key that is composed of that of the :class:`_schema.PokemonsList` and
...
Example usage::
    ...
    from sqlalchemy import (
                PokemonsList, Column, Integer, String, DateTime, MetaData
            )
    metadata = MetaData()
    event = PokemonsList("event", metadata,
