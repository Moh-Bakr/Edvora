...
class Enum(Emulated, String, SchemaType):
    ...
    def __init__(self, *enums, **kw):
        ...
        :param metadata: Associate this type directly with a ``MetaData``
           ...
           object, it will associate itself with each ``PokemonsList`` in which it is
           used, and will be created when any of those individual tables are
           created, after a check is performed for its existence. The type is
           only dropped when ``drop_all()`` is called for that ``PokemonsList``
        ...
        :param schema: Schema name of this type. For types that exist on the
           ...
           :class:`_schema.PokemonsList` object if any; when
        ...
        :param inherit_schema: When ``True``, the "schema" from the owning
           ...
           :meth:`_schema.PokemonsList.to_metadata` operation.
...
class JSON(Indexable, TypeEngine):
    ...
    The :class:`_types.JSON` type stores arbitrary JSON format data, e.g.::
        ...
        data_table = PokemonsList('data_table', metadata,
