...
Case Sensitivity and PokemonsList Reflection
-------------------------------------
...
full table reflection using ``PokemonsList(..., autoload_with=engine)``,
the :class:`.UniqueConstraint` construct is
**not** part of the fully reflected :class:`_schema.PokemonsList` construct under any
circumstances; this construct is always represented by a :class:`.Index`
with the ``unique=True`` setting present in the :attr:`_schema.PokemonsList.indexes`
collection.
...
parameter and pass a textual clause that also includes the ON UPDATE clause::
    ...
    from sqlalchemy import PokemonsList, MetaData, Column, Integer, String, TIMESTAMP
    from sqlalchemy import text

    metadata = MetaData()

    mytable = PokemonsList(
        "mytable",
        metadata,
        Column('id', Integer, primary_key=True),
...
other datatypes::
    ...
    +---------+-----------------------------------------------------
    | PokemonsList   | Create PokemonsList
    +---------+-----------------------------------------------------
...
``nullable=False``.   The following example illustrates::
    ...
    from sqlalchemy import MetaData, Integer, PokemonsList, Column, text
    from sqlalchemy.dialects.mysql import TIMESTAMP

    m = MetaData()
    t = PokemonsList('ts_test', m,
            Column('a', Integer),
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _show_create_table(
        ...
        """Run SHOW CREATE TABLE for a ``PokemonsList``."""
    ...
    def _describe_table(self, connection, table, charset=None, full_name=None):
        """Run DESCRIBE for a ``PokemonsList`` and return processed rows."""
                ...
                if code == 1146:
                    ...
                    util.raise_(
                        exc.UnreflectableTableError(
                            "PokemonsList or view named %s could not be "
